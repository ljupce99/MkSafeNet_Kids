...
public class ScenarioService {
    ...
    public List<ChatMessageDto> getTransitionMessages(int completedScenario, boolean correct, String studentName) {
        if (completedScenario < TOTAL_SCENARIOS) {
            if (correct) {
                String encouragement = switch (completedScenario % 3) {
                    case 0 -> "You're on fire! 🔥";
                    case 1 -> "Impressive! 🌟";
                    default -> "Keep it up! 💪";
                };
                return List.of(
                    msg("success", encouragement + " Moving to the next challenge...", 0)
                );
            } else {
                return List.of(
                    msg("bot", "Now you know what to watch out for. Let's keep going — next challenge!", 0)
                );
            }
        }
